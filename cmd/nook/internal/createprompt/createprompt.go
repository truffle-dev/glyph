// Package createprompt renders nook's "create file or directory" prompt —
// a small modal that asks for a path (relative to a parent directory) and
// emits the value back to the host on Enter. The prompt is a pure value:
// the host owns when it appears, what parent directory it carries, and
// what to do with the typed value on commit.
//
// Path semantics: the user types a relative path. A trailing "/" means
// "create as directory"; otherwise "create as file". Nested input like
// "internal/foo/bar.go" is allowed — the host's create command will
// materialize intermediate directories.
package createprompt

import (
	"strings"
	"unicode"

	"github.com/charmbracelet/lipgloss"

	"github.com/truffle-dev/glyph/components/theme"
)

// minWidth is the smallest outer width we'll try to render at.
const minWidth = 32

// Prompt is the immutable state of the create-prompt modal. New() returns
// a closed prompt; WithParent re-arms it with the parent directory label
// shown in the title.
type Prompt struct {
	open      bool
	parentRel string
	value     []rune
	cursor    int
	err       string
}

// New returns a closed prompt.
func New() Prompt { return Prompt{} }

// WithParent opens the prompt with an empty input and a parent-directory
// label shown in the title. parentRel is a short relative path (e.g. "."
// for the workspace root or "internal/foo" for a nested directory) shown
// to confirm where the new entry will land.
func (p Prompt) WithParent(parentRel string) Prompt {
	p.open = true
	p.parentRel = parentRel
	p.value = nil
	p.cursor = 0
	p.err = ""
	return p
}

// Open reports whether the prompt is currently displayed.
func (p Prompt) Open() bool { return p.open }

// Close returns a closed prompt.
func (p Prompt) Close() Prompt {
	p.open = false
	p.value = nil
	p.cursor = 0
	p.err = ""
	return p
}

// Value returns the current input string (trimmed of leading/trailing space).
func (p Prompt) Value() string { return strings.TrimSpace(string(p.value)) }

// ParentRel returns the parent-directory hint passed into WithParent.
func (p Prompt) ParentRel() string { return p.parentRel }

// WithError surfaces an error message under the input row. Used by the
// host when CreatePath rejects the typed path (collision, invalid name,
// etc.). Typing or backspace clears the error.
func (p Prompt) WithError(msg string) Prompt {
	p.err = msg
	return p
}

// Type appends a rune at the cursor. Non-printable characters are
// dropped. Control characters and ASCII path-illegal characters
// (\0, null) are blocked; everything else (letters, digits, dot,
// underscore, hyphen, slash, plus printable Unicode) is allowed so
// users can type names like ".env" or "internal/foo/bar.go".
func (p Prompt) Type(r rune) Prompt {
	if !isPathRune(r) {
		return p
	}
	p.value = append(p.value[:p.cursor], append([]rune{r}, p.value[p.cursor:]...)...)
	p.cursor++
	p.err = ""
	return p
}

// Backspace deletes the rune to the left of the cursor.
func (p Prompt) Backspace() Prompt {
	if p.cursor == 0 {
		return p
	}
	p.value = append(p.value[:p.cursor-1], p.value[p.cursor:]...)
	p.cursor--
	p.err = ""
	return p
}

// MoveLeft moves the cursor one rune to the left.
func (p Prompt) MoveLeft() Prompt {
	if p.cursor > 0 {
		p.cursor--
	}
	return p
}

// MoveRight moves the cursor one rune to the right.
func (p Prompt) MoveRight() Prompt {
	if p.cursor < len(p.value) {
		p.cursor++
	}
	return p
}

// MoveHome moves the cursor to the start of the line.
func (p Prompt) MoveHome() Prompt { p.cursor = 0; return p }

// MoveEnd moves the cursor to the end of the line.
func (p Prompt) MoveEnd() Prompt { p.cursor = len(p.value); return p }

// View renders the prompt as a bordered modal at the given outer width.
// Returns "" when the prompt is closed.
func (p Prompt) View(t theme.Theme, width int) string {
	if !p.open {
		return ""
	}
	if width < minWidth {
		width = minWidth
	}
	inner := width - 4 // 1+1 border, 1+1 padding
	if inner < 24 {
		inner = 24
	}

	titleLine := "new file or directory"
	if p.parentRel != "" {
		titleLine = "new in " + p.parentRel
	}
	titleStyle := lipgloss.NewStyle().
		Foreground(t.TextMuted).
		Background(t.Surface).
		Width(inner)
	title := titleStyle.Render(clip(titleLine, inner))

	prefix := "› "
	prefixLabel := lipgloss.NewStyle().
		Foreground(t.TextMuted).
		Background(t.Surface).
		Render(prefix)
	used := lipgloss.Width(prefix)
	inputWidth := inner - used
	if inputWidth < 4 {
		inputWidth = 4
	}
	input := renderInput(p.value, p.cursor, t, inputWidth)
	inputRow := lipgloss.NewStyle().
		Background(t.Surface).
		Width(inner).
		Render(prefixLabel + input)

	hint := "enter to create · trailing / for dir · esc to cancel"
	hintStyle := lipgloss.NewStyle().
		Foreground(t.TextMuted).
		Background(t.Surface).
		Width(inner)
	hintRow := hintStyle.Render(clip(hint, inner))

	rows := []string{title, "", inputRow}
	if p.err != "" {
		errStyle := lipgloss.NewStyle().
			Foreground(t.Error).
			Background(t.Surface).
			Width(inner)
		rows = append(rows, errStyle.Render(clip(p.err, inner)))
	}
	rows = append(rows, "", hintRow)

	body := strings.Join(rows, "\n")
	return lipgloss.NewStyle().
		Border(lipgloss.RoundedBorder()).
		BorderForeground(t.Border).
		Background(t.Surface).
		Padding(0, 1).
		Width(inner).
		Render(body)
}

// renderInput draws the current value with a block-style cursor at the
// given rune index. The result is clipped to width.
func renderInput(value []rune, cursor int, t theme.Theme, width int) string {
	textStyle := lipgloss.NewStyle().Foreground(t.Text).Background(t.Surface)
	cursorStyle := lipgloss.NewStyle().Foreground(t.Bg).Background(t.Primary).Bold(true)

	left := string(value[:cursor])
	var caret string
	var right string
	if cursor < len(value) {
		caret = string(value[cursor])
		right = string(value[cursor+1:])
	} else {
		caret = " "
		right = ""
	}
	rendered := textStyle.Render(left) + cursorStyle.Render(caret) + textStyle.Render(right)
	used := lipgloss.Width(string(value))
	if cursor == len(value) {
		used++
	}
	pad := width - used
	if pad > 0 {
		rendered += textStyle.Render(strings.Repeat(" ", pad))
	}
	return rendered
}

// isPathRune returns true for runes that are valid in a path component.
// Permissive: letters (incl. Unicode), digits, and a small set of ASCII
// punctuation that's safe in POSIX paths and ZFS/NTFS path bytes. Slash
// is allowed so users can type nested paths like "internal/foo/bar.go".
// Control characters and null are explicitly rejected.
func isPathRune(r rune) bool {
	if r == 0 {
		return false
	}
	if unicode.IsControl(r) {
		return false
	}
	switch r {
	case '/', '.', '_', '-', ' ':
		return true
	}
	if unicode.IsLetter(r) || unicode.IsDigit(r) {
		return true
	}
	// Allow other printable punctuation commonly used in filenames
	// (~, +, =, @, etc.) without trying to enumerate every legal byte
	// — POSIX path bytes are everything except NUL and "/" which we've
	// already routed.
	if unicode.IsPrint(r) {
		return true
	}
	return false
}

// clip truncates s to the given display-cell width, appending "…" when cut.
func clip(s string, width int) string {
	if width <= 0 {
		return ""
	}
	if lipgloss.Width(s) <= width {
		return s
	}
	if width == 1 {
		return "…"
	}
	var b strings.Builder
	used := 0
	for _, r := range s {
		w := lipgloss.Width(string(r))
		if used+w > width-1 {
			break
		}
		b.WriteRune(r)
		used += w
	}
	b.WriteRune('…')
	return b.String()
}
